...
ResultCode initConnectionData( const ConfigSnapshot* config, ConnectionData* connectionData, StringView userAgentHttpHeader )
{
    ...
    // A8C: Add Atomic site ID fields
    enum { atomicSiteBufferSize = 256 };
    ...
    if ( ! isNullOrEmtpyString( config->atomicSite ) )
    {
        ...
        ELASTIC_APM_CALL_IF_FAILED_GOTO( addToCurlStringList( /* in,out */ &connectionData->requestHeaders, atomicSite ) );
    }
